...
public class TarefaDAO {
    ...
    public long salvarTarefa(Tarefa tarefa)
    {
        ...
        values.put(DatabaseHelper.Tarefas.TAREFA, tarefa.getTarefa());

        if ( tarefa.get_id() != null )
            return getDatabase().update(DatabaseHelper.Tarefas.TABELA, values,
                    "_id = ?", new String[]{ tarefa.get_id().toString() });

        return getDatabase().insert(DatabaseHelper.Tarefas.TABELA,
                null, values);
    }
